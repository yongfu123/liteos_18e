...
#define GE_TEST_INP     1
#define GE_TEST_YXW     1
...
ISP_CMOS_GE_S g_astCmosGeCtx[ISP_MAX_DEV_NUM] = {{0}};
...
static HI_VOID Ge_GetRegCfg(ISP_DEV IspDev, ISP_GE_REG_CFG_S* pstGeReg, HI_U32 iso)
{
	...
	//HI_U16 GE_TEST_INP=1;		
	//HI_U16 GE_TEST_YXW=1;
...
#else
    TmpThr = CLIP3((HI_S32)pstGe->u16Threshold      - (HI_S32)(1<<(pstGe->bitDepth-1)),0,(1<<pstGe->bitDepth)-1);
    if( pstGeReg->ge_th > TmpThr )
    {
    	pstGeReg->ge_th = TmpThr;
    }        
#endif
