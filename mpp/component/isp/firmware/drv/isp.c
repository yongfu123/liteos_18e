...
osal_timeval_t time1;
osal_timeval_t time2;
#endif
/****************************************************************************
...
HI_S32 ISP_GetFrameEdge(ISP_DEV IspDev, HI_U32 *pu32Status)
{
    ...
    s32Ret = osal_wait_event_timeout(&pstDrvCtx->stIspWait, (pstDrvCtx->bEdge == HI_TRUE), (200));
    if (s32Ret <= 0)
    {
        #if 0
        HI_PRINT(   "Wait frame edge timeout!\n");
        #endif
        return -1;
    }
    ...
    HI_U32 u32MilliSec, HI_U32 *pu32status)
{
    ...
    if(osal_down(&pstDrvCtx->stIspSem))
    {
       return -ERESTARTSYS;
    }
    ...
    if(0 != u32MilliSec)
    {
        s32Ret = osal_wait_event_timeout(&pstDrvCtx->stIspWaitVd,
            (HI_TRUE == pstDrvCtx->bVd), (u32MilliSec));
        if (s32Ret <= 0)
        {
            #if 0
            HI_PRINT(   "Wait frame edge timeout!\n");
    ...
    {
        s32Ret = osal_wait_event(&pstDrvCtx->stIspWaitVd, (HI_TRUE == pstDrvCtx->bVd));
        if(s32Ret)
        {
            osal_up(&pstDrvCtx->stIspSem);
            return -1;
...
HI_S32 ISP_DRV_GetSyncControlnfo(ISP_SYNC_CFG_S *pstSyncCfg)
{
    ...
    {
        HI_PRINT("VC number err, can't be large than VC total!\n");
    }
...
HI_S32 ISP_DRV_CalcSyncCfg(ISP_SYNC_CFG_S *pstSyncCfg)
{
    ...
    u64CurExp = osal_do_div64(u64CurExp, DIV_0_TO_1(u64PreExp));
    ...
    ISP_BUS_TYPE_E enType, ISP_BUS_CALLBACK_S *pstBusCb)
{
    ...
    {
        osal_spin_unlock_irqrestore(&g_stIspLock, &u32Flags);
        HI_PRINT("The bus type %d registerd to isp is err!", enType);
...
HI_S32 ISP_DRV_ProcInit(ISP_DEV IspDev)
{
    ...
    if (osal_down(&pstDrvCtx->stProcSem))
    {
        return -ERESTARTSYS;
    }
...
HI_S32 ISP_DRV_ProcPrintf(ISP_DEV IspDev, osal_proc_entry_t *s)
{
    ...
    {
        pszBuf = osal_kmalloc(PROC_PRT_SLICE_SIZE+1);
        if (!pszBuf)
        {
...
HI_S32 ISP_SetDCFInfo(ISP_DEV IspDev,ISP_DCF_INFO_S *pstIspDCF)
{
    ...
    pstDrvCtx = ISP_DRV_GET_CTX(IspDev);
    osal_memcpy(pstDrvCtx->pDCFInfoVirAddr,pstIspDCF,sizeof(ISP_DCF_INFO_S));
    return HI_SUCCESS;
}
...
HI_S32 ISP_GetDCFInfo(ISP_DEV IspDev,ISP_DCF_INFO_S *pstIspDCF)
{
	...
	{
		osal_memcpy(pstIspDCF,pstDrvCtx->pDCFInfoVirAddr,sizeof(ISP_DCF_INFO_S));
	}
...
static long ISP_ioctl(unsigned int cmd, unsigned long arg, void *private_data)
{
    ...
    switch (cmd)
    {
        ...
        {
            ISP_VERSION_S *pVersion = (ISP_VERSION_S *)argp;
        ...
        {
            ISP_VD_TIMEOUT_S *pstVdTimeOut = (ISP_VD_TIMEOUT_S *)argp;

            ISP_GetVdTimeOut(IspDev, &pstVdTimeOut->stVdInfo,
                pstVdTimeOut->u32MilliSec, &pstVdTimeOut->s32IntStatus);
            
            pstVdTimeOut->s32IntStatus &= HW_REG(IO_ADDRESS_PORT(ISP_INT_MASK));
            osal_memcpy(argp, pstVdTimeOut, sizeof(ISP_VD_TIMEOUT_S));
            return 0;
        }
#ifdef ENABLE_JPEGEDCF
        case ISP_DCF_INFO_SET:
        {
            ISP_SetDCFInfo(IspDev, (ISP_DCF_INFO_S *)argp);
            
            return HI_SUCCESS;
        }
        case ISP_DCF_INFO_GET:
        {
            ISP_DCF_INFO_S *pstIspDCF = (ISP_DCF_INFO_S *)argp;

            ISP_GetDCFInfo(IspDev, pstIspDCF); 
            return HI_SUCCESS;
        }
        case ISP_DCFINFO_BUF_INIT :
        {
            s32Ret = ISP_DRV_DCFInfoBufInit(IspDev);
            *argp = g_astIspDrvCtx[IspDev].u32DCFInfoPhyAddr;
            return s32Ret;
        }
        ...
        {
            ISP_STAT_INFO_S *pstStatInfo = (ISP_STAT_INFO_S *)argp;            
        ...
        {          
            return ISP_DRV_StatBufUserPut(IspDev, (ISP_STAT_INFO_S*)argp);
        }

        case ISP_STAT_SHADOW_MEMSTATE_SET:
        {
            g_astIspDrvCtx[IspDev].stStatShadowMem.bUsrAccess = *(HI_BOOL *)argp;
        ...
        {
            * (HI_U32 *)argp = g_astIspDrvCtx[IspDev].stStatShadowMem.u32PhyAddr;           
            return 0;
        }
        ...
        {
            if (osal_down(&g_astIspDrvCtx[IspDev].stProcSem))
            {
                return -ERESTARTSYS;
            }
            return 0;
...
static int ISP_open(void *private_data)
{
    return 0;
}

static int ISP_close(void *private_data)
{
    return 0;
}
...
static int ISP_DRV_Init(void)
{
...
#if 0
    reg_vicap_base_va = (void *)OSAL_IO_ADDRESS(VICAP_BASE);
    if (reg_vicap_base_va == HI_NULL)
    {
        HI_PRINT(  "Remap VI base failed!\n");
        return HI_FAILURE;
    }

    reg_isp_base_va = (void *)OSAL_IO_ADDRESS(ISP_BASE);
    if (reg_isp_base_va == HI_NULL)
    {
        HI_PRINT(  "Remap isp base failed!\n");
        return HI_FAILURE;
    }
#else /*经讨论决定，直接使用VI的映射地址，不需要重复映射*/
    ...
    {
        HI_PRINT(   "ISP Register Interrupt Failed!\n");
        return HI_FAILURE;
    }
    ...
    osal_sema_init(&g_astIspDrvCtx[0].stIspSem,1);
...
static int ISP_DRV_Exit(void)
{
    ...
    osal_sema_destory(&g_astIspDrvCtx[0].stIspSem);
...
static int ISP_ProcShow(osal_proc_entry_t *s)
{
    ...
    osal_seq_printf(s, "%12s" "%12s" "%12s" "%12s" "%12s" "%12s" "%12s" "%12s\n" 
            ,"IspDev","IntCnt","IntT","MaxIntT","IntGapT","MaxGapT","IntRat","IspResetCnt");

    osal_seq_printf(s, "%12d" "%12d" "%12d" "%12d" "%12d" "%12d" "%12d" "%12d\n",
            0,
            pstDrvCtx->stDrvDbgInfo.u32IspIntCnt,
    ...
    osal_seq_printf(s, "%12s" "%12d" "%12d" "%12d" "%12d" "%12d" "%12d" "%12d\n",
            "",
            pstDrvCtx->stDrvDbgInfo.u32PtIntCnt,
...
#ifdef ENABLE_JPEGEDCF
ISP_EXPORT_FUNC_S g_stIspExpFunc = {ISP_RegisterBusCallBack, ISP_RegisterPirisCallBack, ISP_GetDCFInfo};
#else
ISP_EXPORT_FUNC_S g_stIspExpFunc = {ISP_RegisterBusCallBack, ISP_RegisterPirisCallBack};
#endif
