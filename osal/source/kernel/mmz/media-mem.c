...
static int _check_mmz(hil_mmz_t *zone)
{
    ...
    if (new_start<g_sys_mem_addr_end)
    {
        printk(KERN_ERR "ERROR: MMZ start addr(0x%x) is less than OS end addr(0x%x):\n", 
            new_start, g_sys_mem_addr_end);
        return -1;
    }
    #endif
...
static int mmz_write_proc(struct osal_proc_dir_entry *entry, char *buffer, int count)
{
	char buf[256];
