...
extern void *osal_kmalloc(unsigned long size);
...
extern int osal_wait(osal_wait_t * wait);
extern int osal_wait_uninterruptible(osal_wait_t * wait);
extern int osal_wait_timeout(osal_wait_t * wait, unsigned long ms);
//dev_wait_event-->wait_event_interruptible
#define osal_wait_event(wait, condition)    \
({                                  \
	int __ret = 0;                          \
                                          \
    for (;;){                          \
		...
		__ret = osal_wait_timeout(wait,100);   \
		if(__ret < 0)                 \
			break;           \
    }                                    \
	__ret;                                   \
})
#define osal_wait_event_uninterruptible(wait, condition)    \
({                                  \
    int __ret = 0;                          \
                                          \
    for (;;){                          \
        ...
        __ret = osal_wait_uninterruptible(wait);   \
        if(__ret < 0)                 \
            break;           \
    }                                    \
    __ret;                                   \
...
#define osal_wait_event_timeout(wait, condition, timeout)             \
({                                  \
    int __ret = timeout;                          \
    ...
    for (;;) {                          \
        if (condition)                       \
			break;                    \
		__ret = osal_wait_timeout(wait, __ret);   \
...
extern void osal_kthread_destory(osal_task_t *task);
...
typedef char *osal_va_list;

/*
 * Storage alignment properties
 */
#define OSAL_AUPBND (sizeof (int) - 1)
#define OSAL_ADNBND (sizeof (int) - 1)

/*
 * Variable argument list macro definitions
 */
#define osal_bnd(X, bnd) (((sizeof (X)) + (bnd)) & (~(bnd)))
#define osal_va_arg(ap, T) (*(T *)(((ap) += (osal_bnd (T, OSAL_ADNBND))) - (osal_bnd (T,OSAL_ADNBND))))
#define osal_va_end(ap) (void) 0
#define osal_va_start(ap, A) (void) ((ap) = (((char *) &(A)) + (osal_bnd (A,OSAL_ADNBND))))
