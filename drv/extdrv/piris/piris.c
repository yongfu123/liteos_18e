...
#endif
{
    ...
    //LOS_SwTmrCreate(u32Interval, OS_SWTMR_MODE_PERIOD, piris_timer_cb, &p_piris_dev->u16TimerID, (UINT32)p_piris_dev);
    //LOS_SwTmrStart(p_piris_dev->u16TimerID);
    LOS_SwtmrCreate(u32Interval, LOS_SWTMR_MODE_PERIOD, piris_timer_cb, &p_piris_dev->u16TimerID, (UINT32)p_piris_dev);
    LOS_SwtmrStart(p_piris_dev->u16TimerID);    
#else
...
{
#ifdef __HuaweiLite__
    LOS_SwTmrDelete(p_piris_dev->u16TimerID);
#else
...
static int piris_probe(struct platform_device *pdev)
{
    ...
    reg_pirisI_base_va = devm_ioremap_resource(&pdev->dev, mem);
    if (IS_ERR(reg_pirisI_base_va))
            return PTR_ERR(reg_pirisI_base_va);
