...
int mipi_drv_init_phy_power_ctl(unsigned int en)
{
    ...
    /***************************************************************
    (1)	首先配置sensor
    (2)	PHYCIL_PWDN 寄存器 (地址0x1008) 写1
    (3)	延迟至少20us，PHYCIL_RSTN 寄存器 (地址0x100C) 写1
    (4)	CSI2_RSTN 寄存器 (地址0x0004) 写1
    ****************************************************************/
    /* 1. phy power up */
    for (i = 0; i < COMBO_MAX_LINK_NUM; i++)
    {
...
void mipi_link_int_statics(COMBO_LINK link_id)
{
    ...
    /*PHYCIL_INT_STAT 读后不会清零,需要向PHYCIL_INT_CLR写入1才会清零*/
    if (link_int_status)
    {
